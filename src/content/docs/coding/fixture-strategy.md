...
根目录 conftest.py 放全局 Fixture（环境配置、日志初始化），各模块目录的 conftest.py 放模块专用 Fixture（该模块的登录态、数据准备）。命名上，通用 Fixture 用 base_ 前缀，业务 Fixture 用业务场景前缀。这样既避免全局膨胀，又保证 Fixture 作用域清晰。
