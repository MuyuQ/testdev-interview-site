...
selfTests:
  - id: "ab-testing-check-1"
    question: "A/B 测试最先要明确什么？"
    options:
      - "实验假设、指标和分组策略"
      - "页面颜色是否足够醒目"
      - "只看实验组转化率"
      - "先全量上线再观察"
    correctIndex: 0
    explanation: "A/B 测试要先明确可验证的假设、核心指标、分组策略和样本量要求，否则结果很难被解释。"
  - id: "ab-testing-check-2"
    question: "为什么 A/B 测试不能样本量很小时就下结论？"
    options:
      - "样本量小会让统计显著性不足，结果可能只是随机波动"
      - "样本量小一定说明方案无效"
      - "样本量小会导致代码无法发布"
      - "样本量小只影响页面性能"
    correctIndex: 0
    explanation: "样本量不足会提高误判风险，必须结合显著性、功效和置信区间判断结果可信度。"
  - id: "ab-testing-check-3"
    question: "A/B 测试与灰度发布的核心区别是什么？"
    options:
      - "A/B 测试验证方案效果，灰度发布控制发布风险"
      - "A/B 测试只用于后端，灰度发布只用于前端"
      - "A/B 测试不需要数据，灰度发布需要统计检验"
      - "两者没有区别"
    correctIndex: 0
    explanation: "A/B 测试关注产品或策略效果，灰度发布关注新版本稳定性和风险控制，两者可以配合但目的不同。"
relatedSlugs: ["glossary/data-driven-testing", "canary-release"]
termLinks:
