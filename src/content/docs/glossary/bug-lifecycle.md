...
selfTests:
  - id: "bug-lifecycle-check-1"
    question: "Bug 新建后通常下一步是什么？"
    options:
      - "开发确认是否为真实缺陷"
      - "测试直接关闭"
      - "立即发布到生产"
      - "删除缺陷记录"
    correctIndex: 0
    explanation: "新建 Bug 需要开发确认和分析，确认后再进入修复流转。"
  - id: "bug-lifecycle-check-2"
    question: "Bug 可以关闭前至少要确认什么？"
    options:
      - "问题不复现、无新增问题、影响场景验证通过"
      - "开发说已经修复即可"
      - "只看截图是否消失"
      - "只要版本能构建成功"
    correctIndex: 0
    explanation: "关闭 Bug 是测试责任，需要验证原问题、相关场景和修复影响，不能只听口头结论。"
  - id: "bug-lifecycle-check-3"
    question: "reopen 的 Bug 为什么需要重点关注？"
    options:
      - "它说明之前的修复或验证链路存在问题"
      - "它一定不是 Bug"
      - "它不需要再验证"
      - "它只能由产品关闭"
    correctIndex: 0
    explanation: "reopen 往往代表修复不彻底、理解偏差或回归缺陷，需要追溯根因并改进流程。"
