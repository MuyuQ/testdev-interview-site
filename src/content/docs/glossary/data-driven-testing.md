...
selfTests:
  - id: "data-driven-testing-check-1"
    question: "数据驱动测试最适合哪类场景？"
    options:
      - "输入输出组合多且测试逻辑重复"
      - "只需要人工体验的探索场景"
      - "完全没有预期结果的场景"
      - "只验证页面颜色的场景"
    correctIndex: 0
    explanation: "数据驱动适合规则校验、边界值和参数组合多的场景，一条逻辑配多组数据能降低重复代码。"
  - id: "data-driven-testing-check-2"
    question: "数据文件纳入仓库管理的主要价值是什么？"
    options:
      - "让测试数据和代码同步变更、可评审、可追溯"
      - "让测试一定更慢"
      - "避免写任何断言"
      - "替代所有测试环境"
    correctIndex: 0
    explanation: "数据文件是测试资产，纳入版本管理后能跟随代码变更并被评审，减少环境和版本不一致。"
  - id: "data-driven-testing-check-3"
    question: "数据驱动失败时为什么要输出 case_id？"
    options:
      - "便于快速定位是哪组数据失败"
      - "为了隐藏失败原因"
      - "为了让失败自动通过"
      - "为了减少测试覆盖"
    correctIndex: 0
    explanation: "每条数据有唯一标识，报告中展示 case_id 能快速定位输入、预期和实际差异。"
