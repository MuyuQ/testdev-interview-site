...
selfTests:
  - id: "error-guessing-check-1"
    question: "错误猜测法应该如何定位自己的角色？"
    options:
      - "作为正式测试设计方法的补充"
      - "完全替代等价类和边界值"
      - "只用于记录缺陷编号"
      - "只适合不懂业务的人使用"
    correctIndex: 0
    explanation: "错误猜测法依赖经验，适合补充正式方法遗漏的高风险场景，但不能替代系统化覆盖。"
  - id: "error-guessing-check-2"
    question: "一个有依据的错误猜测通常来自哪里？"
    options:
      - "历史缺陷、业务复杂度、技术难点和变更类型"
      - "随机点击页面"
      - "只看界面颜色"
      - "完全不看需求"
    correctIndex: 0
    explanation: "成熟的错误猜测不是凭空想象，而是从历史数据、实现风险和业务规则中提炼线索。"
  - id: "error-guessing-check-3"
    question: "为什么要沉淀错误猜测清单？"
    options:
      - "把个人经验转化为团队可复用资产"
      - "让所有测试都跳过需求分析"
      - "替代缺陷管理工具"
      - "让测试报告更短"
    correctIndex: 0
    explanation: "清单能复用有效经验，帮助团队持续覆盖曾经暴露过的缺陷模式。"
