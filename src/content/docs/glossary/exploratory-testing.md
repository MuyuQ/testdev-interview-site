...
selfTests:
  - id: "exploratory-testing-check-1"
    question: "探索性测试为什么不是随意测试？"
    options:
      - "它需要目标、方法和记录"
      - "它完全不需要测试人员判断"
      - "它只执行自动化脚本"
      - "它不能发现新问题"
    correctIndex: 0
    explanation: "探索性测试强调边学习边执行，但仍要设定探索目标、选择方法并记录发现。"
  - id: "exploratory-testing-check-2"
    question: "探索性测试与预设测试的关系是什么？"
    options:
      - "互补关系，预设测试保已知功能，探索测试发现未知问题"
      - "探索性测试完全替代回归测试"
      - "预设测试只适合线上环境"
      - "两者不能在同一版本使用"
    correctIndex: 0
    explanation: "预设测试保证明确需求和历史回归，探索性测试补充文档未覆盖的边界、异常和体验问题。"
  - id: "exploratory-testing-check-3"
    question: "探索性测试结束后应该做什么？"
    options:
      - "总结探索路径、缺陷和可补充的用例"
      - "删除所有记录"
      - "只口头告诉开发"
      - "不需要再回归验证"
    correctIndex: 0
    explanation: "探索发现要沉淀为缺陷、用例或检查清单，才能被团队复用并进入后续回归体系。"
