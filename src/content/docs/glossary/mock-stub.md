...
selfTests:
  - id: "mock-stub-check-1"
    question: "Stub 的主要作用是什么？"
    options:
      - "返回预设数据，让被测逻辑可继续执行"
      - "记录完整生产流量"
      - "替代所有集成测试"
      - "自动修复外部服务"
    correctIndex: 0
    explanation: "Stub 关注提供固定返回值，帮助测试隔离外部依赖并验证内部逻辑。"
  - id: "mock-stub-check-2"
    question: "Mock 相比 Stub 多了什么能力？"
    options:
      - "验证调用次数、参数和交互行为"
      - "自动生成真实数据库"
      - "替代需求评审"
      - "让测试无需断言"
    correctIndex: 0
    explanation: "Mock 不仅能返回数据，还能记录调用并验证外部交互是否符合预期。"
  - id: "mock-stub-check-3"
    ...
    options:
      ...
      - "单元测试会变成端到端测试"
    correctIndex: 0
    explanation: "Mock 与真实实现不一致会产生假安全感，因此关键链路还需要契约测试、集成测试或真实环境验证。"
