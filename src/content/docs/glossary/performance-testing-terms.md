...
selfTests:
  - id: "performance-testing-terms-check-1"
    question: "负载测试主要验证什么？"
    options:
      - "系统在预期并发和业务量下是否稳定"
      - "系统在完全无用户时的界面样式"
      ...
      - "需求文档是否排版正确"
    correctIndex: 0
    explanation: "负载测试关注预期压力下的响应时间、吞吐量、错误率和资源使用是否满足目标。"
  - id: "performance-testing-terms-check-2"
    question: "压力测试与负载测试的主要区别是什么？"
    options:
      - "压力测试会持续加压寻找系统极限和失效点"
      - "压力测试只检查代码格式"
      - "负载测试必须在生产执行"
      - "两者完全相同"
    correctIndex: 0
    explanation: "压力测试关注突破预期容量后的瓶颈和降级表现，负载测试关注正常目标负载下是否达标。"
  - id: "performance-testing-terms-check-3"
    question: "分析性能结果时为什么不能只看平均响应时间？"
    options:
      - "平均值会掩盖 P95/P99 等长尾体验"
      - "平均值无法计算"
      - "平均值只适合 UI 测试"
      - "平均值越大越好"
    correctIndex: 0
    explanation: "性能体验常受长尾请求影响，P95/P99、错误率和吞吐量要一起看。"
