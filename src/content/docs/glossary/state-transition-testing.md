...
selfTests:
  - id: "state-transition-testing-check-1"
    question: "状态迁移测试首先要梳理什么？"
    options:
      - "状态集合、合法迁移、触发条件和非法迁移"
      - "页面配色"
      - "测试报告标题"
      - "开发人员姓名"
    correctIndex: 0
    explanation: "状态迁移测试要先建立状态模型，明确哪些转换合法、哪些转换必须被拒绝。"
  - id: "state-transition-testing-check-2"
    question: "为什么要验证非法状态迁移？"
    options:
      - "防止系统接受越权、倒退或跳跃的状态变化"
      - "为了减少测试覆盖"
      - "为了让流程更复杂"
      - "因为合法迁移不需要测试"
    correctIndex: 0
    explanation: "非法迁移常导致订单、审批、支付等核心数据状态错乱，必须明确拒绝和回滚策略。"
  - id: "state-transition-testing-check-3"
    question: "并发场景对状态迁移测试的主要风险是什么？"
    options:
      - "多个操作同时触发导致最终状态不一致"
      - "页面加载字体变大"
      - "测试数据无法命名"
      - "断言数量过少"
    correctIndex: 0
    explanation: "并发支付、取消、回调等操作可能竞争同一状态，需要验证幂等、锁和最终一致性。"
