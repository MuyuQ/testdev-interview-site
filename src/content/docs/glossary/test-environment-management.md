...
selfTests:
  - id: "test-environment-management-check-1"
    question: "测试环境管理的核心目标是什么？"
    options:
      - "让环境稳定、可复现、可隔离"
      - "让所有人共用同一份脏数据"
      - "让测试只能手工部署"
      - "让环境变化不可追溯"
    correctIndex: 0
    explanation: "好的测试环境要保证配置、数据和依赖可控，问题能复现且不同团队互不干扰。"
  - id: "test-environment-management-check-2"
    question: "为什么测试环境需要配置版本管理？"
    options:
      - "便于追踪环境变更并快速回滚"
      - "为了避免任何自动化"
      - "为了隐藏真实配置"
      - "为了让环境无法复现"
    correctIndex: 0
    explanation: "配置纳入版本管理后，环境差异和变更原因可追溯，出现问题时也更容易回滚。"
  - id: "test-environment-management-check-3"
    question: "多团队共用测试环境时最需要防范什么？"
    options:
      - "数据、配置和服务版本互相污染"
      - "文档目录太清晰"
      - "测试报告太完整"
      - "缺陷记录过多字段"
    correctIndex: 0
    explanation: "共享环境容易出现数据覆盖、配置冲突和服务版本不一致，需要隔离策略和预约机制。"
