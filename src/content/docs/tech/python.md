...
class HttpClient:
    ...
    def request(
        ...
        try:
            ...
            logger.info(
                f"请求完成: {response.status_code}, 耗时={elapsed:.3f}s"
            )
            ...
            logger.error(f"请求异常: {method} {url}, error={e}")
            raise
...
# 加载数据文件
login_cases = load_test_data("test_data/login_cases.yaml")
...
def test_login(case, api_client):
    ...
    assert resp["body"]["success"] == case["expected"]["success"]
```
...
【继承】把通用操作封装为 BasePage 类（[页面对象模式](/testdev-interview-site/glossary/page-object-pattern/)），具体页面继承 BasePage，复用通用方法（等待元素、截图等）。
