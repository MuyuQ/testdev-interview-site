...
export function initializeSelfTestQuiz(root: ParentNode = document): void {
  root.querySelectorAll<HTMLElement>("[data-quiz]").forEach((quiz) => {
    ...
    quiz
      .querySelectorAll<HTMLButtonElement>(".quiz-check-btn")
      .forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          const selected =
            question.querySelector<HTMLInputElement>("input:checked");
          const explanation =
          ...
          const options =
            question.querySelectorAll<HTMLElement>(".quiz-option");
    ...
    const resetButton =
      quiz.querySelector<HTMLButtonElement>(".quiz-reset-btn");
