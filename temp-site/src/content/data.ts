...
export const glossaryTopics: GlossaryTerm[] = [
    ...
    sections: [
      // ========== 模块 1：基础入门 ==========
      {
        ...
        content: [
          ...
          { type: "text", content: "\n\n" },
          { type: "text", content: "回归测试可以手动执行，也可以自动化执行。在现代软件研发实践中，回归测试主要通过自动化测试套件来完成。测试团队会维护一套回归测试用例集，每次代码变更后自动运行这些用例，检查是否有功能被破坏。\\n\\n如果测试全部通过，说明变更没有引入回归问题；\\n\\n如果有用例失败，就需要排查是否是新代码导致的问题。" },
          { type: "text", content: "\n\n" },
  ...
  },
  {
    ...
    definition:
      "冒烟测试源自硬件测试传统——通电后看是否冒烟。在软件测试中，冒烟测试是发布或提测后的首轮快速验证，覆盖核心路径和关键功能（如登录、下单、支付）。\\n\\n如果冒烟测试失败，后续深度测试就没有意义，需要先修复基础问题。冒烟测试通常自动化执行，在 CI 流水线中作为合并前或发布前的门禁。",
    whyItMatters:
