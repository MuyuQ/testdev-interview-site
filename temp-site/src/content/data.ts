...
export const glossaryTopics: GlossaryTerm[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        title: "常见误区",
        kind: "qa-list",
        items: [{question: "误区1：等价类划分太粗糙", answer: "问题：把所有有效值归为一个等价类，遗漏业务细分场景。\n\n正确做法：按业务逻辑细分等价类，如年龄输入不只是「有效/无效」，还应细分「未成年」「青年」「中年」「老年」等业务等价类。"}, {question: "误区2：忽略无效等价类", answer: "问题：只关注有效输入，忽略非法输入的处理逻辑验证。\n\n正确做法：无效等价类同样重要，每种非法情况要单独设计测试用例，验证系统的错误处理能力。"}, {question: "误区3：等价类假设不加验证", answer: "问题：盲目假设等价类内所有值结果相同，不做验证。\n\n正确做法：关键等价类要用多个值验证假设是否成立，特别是涉及计算逻辑时。"}, {question: "误区4：无效等价类组合测试", answer: "问题：一个测试用例同时测试多个无效等价类，无法定位具体问题。\n\n正确做法：每个无效等价类单独测试，一个用例只验证一种非法情况，其他输入保持有效。"}, {question: "误区5：忽略输出等价类", answer: "问题：只关注输入等价类，忽略输出等价类的划分。\n\n正确做法：等价类划分同样适用于输出，根据输出结果的类型和范围划分等价类，确保输出覆盖完整。"}],
      },
      {
        id: "interview-qa",
        title: "面试问答",
  ...
  },
  {
    ...
    answerHints: [
      ...
      "状态转换导致问题的典型例子：订单状态并发冲突问题。问题描述：用户在手机端点击「取消订单」，同时商家在后台点击「发货」，最终订单状态变成「已发货」但用户已经取消。根因分析：取消和发货两个操作同时执行，状态更新顺序问题导致状态不一致。取消操作把状态改为「已取消」，发货操作把状态改为「已发货」，由于没有状态锁定机制，后执行的操作覆盖了前执行的操作，最终状态是「已发货」。\n\n正确做法：状态转换前要先判断当前状态，只有在允许的状态才能转换。发货操作应该检查当前状态是「已支付」才能发货，如果当前状态已经是「已取消」则拒绝发货。另一个典型例子：订单状态遗漏导致流程卡住。问题描述：订单支付成功后，支付回调由于网络问题没有到达，订单状态一直是「待支付」，用户无法看到订单已支付。根因分析：状态转换只设计了正常路径（支付成功回调→状态变为已支付），没设计异常路径（回调失败怎么处理）。\n\n正确做法：设计状态转换的异常处理机制，包括：回调失败重试、超时主动查询支付状态、状态不一致告警和人工处理流程。面试时要说明：状态转换问题的高发区是「并发操作」「异常路径」「状态不一致」。测试时不能只测正常流转，必须覆盖并发场景和异常场景，验证状态一致性。同时要和开发对齐状态转换图，作为测试和开发的共同参考。",
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            question: "追问应对",
            answer: "追问：遇到过其他坑吗？\n\n回答：还有全局变量污染问题。测试用例之间不应该共享可变全局变量，会导致用例相互影响。解决方法是用 Fixture 注入依赖，或使用上下文管理器。\n\n追问：怎么学习这些坑？\n\n回答：多写代码，踩坑后记录下来。Code Review 时互相检查。看开源项目的 issue 和 bug 修复记录。",
          },
        ],
      },
      {
      ...
      // 模块 3：实操案例
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "项目结构：api/目录存放接口封装（每个接口一个类，包含请求方法和参数校验）；cases/目录存放测试用例（按业务模块组织）；data/目录存放测试数据（YAML 或 JSON 格式）；utils/目录存放工具函数（断言封装、日志、数据库操作）；config/目录存放环境配置（不同环境的域名、账号等）。\n\n核心流程：测试入口调用用例 -> 用例从数据层读取测试数据 -> 用例调用业务层发送请求 -> 业务层调用接口层构造请求 -> 接口层返回响应 -> 用例执行断言。\n\n关键要点：接口封装层隔离请求细节，用例层只关注业务逻辑；数据驱动支持一套代码覆盖多组数据；配置分离支持一键切换环境。",
          },
          {
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            question: "追问应对",
            answer: "追问：发现 CPU 使用率高怎么办？\n\n回答：先用 top 按 P 键找到占用 CPU 最高的进程，判断是正常业务负载还是异常进程。如果是业务进程，分析是代码效率问题还是资源不足。\n\n追问：内存和磁盘怎么看？\n\n回答：内存看 free -h 的 available 列，磁盘看 df -h 的空间使用率，IO 看 iostat 的 %util。",
          },
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          {
            question: "追问应对",
            answer: "追问：chmod 755 是什么意思？\n\n回答：755 是三组权限数字：第一位 7 表示用户权限（读+写+执行）；第二位 5 表示组权限（读+执行）；第三位 5 表示其他用户权限（读+执行）。\n\n追问：遇到 Permission denied 怎么办？\n\n回答：先判断是用户权限不足还是文件权限问题。如果是用户权限不足，检查是否需要 sudo；如果是文件权限问题，用 chmod 添加权限或用 chown 修改所有者。",
          },
        ],
      },
      {
