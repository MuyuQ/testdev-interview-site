    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: "text", content: "\n\n" },
          { type: "text", content: "这套处理流程在面试时要能详细描述，体现对回归测试运维的实战经验。\n\n关键是要说明：不是简单重跑，而是先定位根因，再针对性处理。" },
        ],
      },
      // ========== 模块 4：常见误区 ==========
      {
      ...
      // ========== 模块 5：面试问答 ==========
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "回归时间和覆盖率的平衡围绕「分级策略」和「优先级排序」。分级回归控制时间成本：小变更跑冒烟，模块变更跑增量，发布前跑全量。用例优先级排序保证核心覆盖：核心业务用例先跑，次要业务后跑，时间紧张时可以先跑核心用例。自动化与手动配合：自动化覆盖高频场景，手动覆盖新功能和复杂场景。覆盖率目标分级设定：核心模块 80%+，次要模块 50%+。\n\n关键是「核心功能必覆盖、时间成本可接受」。",
          },
          {
        ...
        items: [
          {
            ...
            answer: "首先确定失败时间点和请求标识（trace_id），然后在日志平台按时间和标识检索。分析思路：从错误日志入手确认异常类型，向上追踪请求链路找到根因，向下检查数据库、缓存、外部服务日志。\n\n关键是日志要有足够的上下文（请求参数、用户信息、环境标识），否则难以定位。",
          },
          {
        ...
        items: [
          {
            ...
            answer: "使用 Git Flow 或简化版的分支策略：main 分支是生产代码，develop 分支是开发基线，feature 分支开发新功能，release 分支准备发布，hotfix 分支修复紧急问题。测试代码遵循相同策略，与业务代码分支对应。\n\n关键是每个分支的用途和合并规则要清晰。",
          },
          {
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          {
            question: "你怎么验证异步接口的最终结果？",
            answer: "验证策略取决于业务场景：对于轮询场景，设计合理的等待间隔和最大等待时间，通过查询接口检查业务状态；对于回调场景，监听回调通知验证参数和签名，再验证业务状态。\n\n关键是明确\"最终状态\"的定义：数据库状态、业务状态、下游通知都要验证。",
          },
          {
            question: "回调接口怎么测？",
    ...
    sections: [
      {
        ...
        kind: "paragraph",
        content: [
          { type: "text", content: "AI 辅助接口测试是把 AI 用于用例生成、请求构造、日志解释和回归分析，但核心判断仍由测试人员把关。接口测试的输入输出结构清晰，最适合率先接入 AI 辅助能力。\n\n关键是建立「AI 辅助、人工决策」的协作模式，既享受提效红利，又守住质量底线。" },
        ],
      },
      {
        id: "learning-path",
    ...
    sections: [
      {
        ...
        kind: "paragraph",
        content: [
          { type: "text", content: "云原生、容器化和可观测性是现代测试环境的基础设施。测试开发不需要成为平台专家，但需要掌握足够的环境部署、日志查看和问题追踪能力。\n\n核心是从测试工作场景出发，学能用的、用能落地的，不追求平台全栈。" },
        ],
      },
      {
        id: "learning-path",
    ...
    sections: [
      {
        ...
        kind: "paragraph",
        content: [
          { type: "text", content: "AI 辅助文档生成是用 AI 生成文档初稿，人工精修结构和关键表述，降低文档维护负担。测试文档更新频繁但优先级常被压后，AI 能显著降低维护成本。\n\n关键是建立「AI 初稿 + 人工精修」的流程，保证文档质量。" },
        ],
      },
      {
        id: "learning-path",
    ...
    sections: [
      {
        ...
        kind: "paragraph",
        content: [
          { type: "text", content: "测试开发学习 AI 工具是建立测试开发场景下的 AI 工具地图，知道什么场景用什么工具。AI 工具层出不穷，需要聚焦与测试开发工作直接相关的场景，以解决问题为导向，不盲目追新。\n\n关键是建立个人 AI 工具清单，记录每个工具的适用场景和局限性。" },
        ],
      },
      {
        id: "learning-path",
