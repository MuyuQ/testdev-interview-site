        ...
        items: [
          {
            ...
            answer: "数据准备方式：通过 API 创建（走业务逻辑，数据一致性好）、数据库插入（快速，适合批量数据）、配置文件定义（静态数据）。\n\n管理策略：按场景分类（基础数据、业务数据、边界数据）、统一数据工厂类生成、数据模板支持参数化覆盖。\n\n关键是数据准备要可重复、可追溯。",
          },
          {
        ...
        items: [
          {
            ...
            answer: "使用 OpenAPI（Swagger）规范定义接口文档，包含：接口路径、请求方法、参数定义、响应结构、错误码。\n\n文档管理方式：代码注解自动生成（保持同步）、独立文档仓库（集中管理）、文档平台（支持在线测试）。\n\n关键是文档与代码同步更新，避免文档过时。",
          },
          {
