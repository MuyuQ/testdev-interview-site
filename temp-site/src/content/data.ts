        ...
        items: [
          {
            ...
            answer: "所有 Fixture 都设为 function 作用域会导致每个测试都重新初始化，执行时间大幅增加。\n\n正确做法是：根据资源成本和隔离需求选择作用域，登录态用 session（成本高、无需隔离）、数据库连接用 module（成本中等、模块内复用）、测试数据用 function（隔离需求高）。面试时要讲清作用域选择原则和性能权衡。",
          },
          {
            question: "误区二：Fixture 嵌套太深",
            answer: "Fixture 嵌套层级过多会让依赖关系难以追踪，阅读成本高。\n\n正确做法是：控制嵌套深度在 2-3 层，复杂依赖拆成多个独立 Fixture 并通过参数注入组合。例如登录态 Fixture 和订单数据 Fixture 独立定义，场景 Fixture 通过参数注入组合两者，而不是在一个 Fixture 里做所有准备。面试时要举一个 Fixture 依赖设计的具体案例。",
          },
          {
            question: "误区三：混入大量业务逻辑",
            answer: "Fixture 应专注于环境准备和资源管理，如果混入业务逻辑会导致难以复用和维护。\n\n正确做法是：Fixture 只做环境层（登录态、数据库、浏览器），业务数据准备放在测试代码或 helper 函数。例如 Fixture 负责创建订单记录，但订单金额计算逻辑应该放在 helper 函数中。面试时要讲清 Fixture 与业务逻辑的边界划分。",
          },
          {
            question: "误区四：忽略资源清理",
      ...
      // ========== 模块 5：面试问答 ==========
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "pytest 通过参数名匹配实现依赖注入。测试函数或 Fixture 的参数名如果与某个 Fixture 同名，pytest 会自动注入该 Fixture 的返回值。例如定义一个 auth_token 的 Fixture 后，任何测试函数只要参数名为 auth_token，就能自动获取登录态。\n\n依赖注入的好处是：测试代码无需显式调用 Fixture，依赖关系清晰；Fixture 可以链式依赖，形成依赖树。面试时可以举例：test_order Fixture 依赖 test_user，只需把 test_user 作为参数传入，pytest 会先执行 test_user Fixture 再执行 test_order。",
          },
          {
            question: "conftest.py 的作用是什么？",
            answer: "conftest.py 是 pytest 的局部配置目录，用于存放可复用的 Fixture 和配置。放在 conftest.py 中的 Fixture 可以被同目录及子目录下的所有测试文件使用，无需显式导入。\n\n好处是：Fixture 集中管理、测试文件无需 import、目录结构清晰。通常做法是：项目根目录的 conftest.py 放全局 Fixture（如数据库连接），模块目录的 conftest.py 放模块级 Fixture（如特定业务的测试数据）。面试时要说明：conftest.py 是 pytest 工程化的重要组成部分。",
          },
        ],
      },
    ],
      ...
      // ========== 模块 4：常见误区 ==========
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "测试环境性能数据不能直接映射到生产环境，因为硬件、数据量、网络条件不同。\n\n正确做法是：性能测试数据要结合环境差异分析，关键指标要留安全余量。比如测试环境 4 核 CPU 测出吞吐量 500 QPS，生产环境 16 核 CPU，估算生产吞吐量约 2000 QPS，但实际要生产验证。",
          },
          {
        ...
        items: [
          {
            ...
            answer: "共用一套环境会导致互相干扰、测试结果不稳定、问题定位困难。\n\n正确做法是：按团队或项目隔离测试环境，或使用动态环境（每次测试临时创建）。比如团队 A 和团队 B 同时测试，团队 A 修改了配置或数据，团队 B 的测试会失败。面试时要说明环境隔离策略和 Implementation 方案。",
          },
          {
            question: "误区二：测试环境与生产环境配置不一致",
            answer: "配置不一致会导致「测试通过、生产失败」的问题。\n\n正确做法是：通过配置模板或配置中心统一管理，测试环境配置与生产保持同步。比如使用 Apollo、Nacos 等配置中心，所有环境从配置中心读取配置，配置变更时所有环境自动同步。面试时要强调配置一致性保障机制。",
          },
          {
            question: "误区三：测试环境数据与生产数据混乱",
            answer: "如果测试环境直接使用生产数据或数据格式不一致，会导致测试结果不可控。\n\n正确做法是：测试数据独立管理，通过数据工厂或数据脚本生成可控测试数据。生产数据需要脱敏后才能导入测试环境，敏感字段（手机号、身份证）要替换成测试数据。面试时要说明数据管理策略和安全合规要求。",
          },
          {
            question: "误区四：环境变更不通知",
            answer: "环境配置变更、服务重启、数据清理如果不通知，会导致其他团队测试失败。\n\n正确做法是：建立环境变更通知机制（如群机器人通知），环境变更前评估影响范围，变更后通知相关团队。重要变更（如数据库结构变更）需要提前协调时间。",
          },
          {
            question: "误区五：忽略环境健康检查",
            answer: "环境服务异常、资源耗尽如果不及时发现，会导致测试大量失败。\n\n正确做法是：部署环境健康检查（如定时调用健康检查接口），配置监控告警（服务不可用、资源超限），环境问题能及时发现和处理。环境稳定性是测试效率的基础。",
          },
        ],
      },
      // ========== 模块 5：面试问答 ==========
        ...
        items: [
          {
            ...
            answer: "如果 Bug 状态随意跳转（如新建直接关闭），会导致问题遗漏或责任不清。\n\n正确做法是：严格按照流程流转，每个状态变更要有明确原因记录。比如新建→确认→修复中→待验证→验证通过→关闭，不能跳过中间状态。面试时要说明你们团队的 Bug 流程规范。",
          },
          {
            question: "误区二：验证通过后 Bug 还不关闭",
            answer: "验证通过后应该及时关闭 Bug，否则 Bug 系统会积累大量已解决问题。\n\n正确做法是：关闭 Bug 是测试的责任，关闭时要记录验证方法和结论。比如「验证通过，库存校验功能正常，相关场景已验证」。",
          },
          {
            question: "误区三：reopen 的 Bug 不追溯根因",
            answer: "如果 Bug reopen 后只是再次修复，可能掩盖更深层问题。\n\n正确做法是：reopen 的 Bug 要追溯根因（修复不彻底、理解偏差、回归问题），并记录教训。比如「修复代码没有合入主分支，导致发版后问题复现」，改进措施是「修复必须合入主分支」。",
          },
          {
            question: "误区四：Bug 报告信息不完整",
            answer: "Bug 报告缺少复现步骤、环境信息、截图日志，会导致开发无法复现和定位。\n\n正确做法是：Bug 报告要包含完整信息（标题、描述、复现步骤、预期结果、实际结果、环境、优先级、附件）。",
          },
          {
            question: "误区五：Bug 优先级设置不合理",
            answer: "所有 Bug 都设为高优先级会导致开发无法判断处理顺序。\n\n正确做法是：按影响范围设置优先级（P0 阻塞性、P1 严重、P2 一般、P3 轻微），P0 和 P1 优先处理，P3 可以延后处理。",
          },
        ],
      },
      // ========== 模块 5：面试问答 ==========
      ...
      // ========== 模块 4：常见误区 ==========
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "错误猜测法依赖个人经验，如果每次只是凭直觉猜测而不记录沉淀，经验无法传承给团队。\n\n好的做法是：把猜测到的缺陷类型和有效场景记录成检查清单，逐步形成团队经验库。比如团队发现「金额输入0.01边界值经常出错」，把这个场景加入检查清单，后续测试时系统化使用。面试时要说明如何把个人经验转化为团队共享的知识。"
          },
          {
            question: "错误猜测范围太窄或太泛",
            answer: "范围太窄是指只关注自己熟悉的错误类型，忽略其他易错区域。比如只关注功能缺陷，忽略性能、安全、兼容性问题。范围太泛是指没有重点地随机猜测，效率低下。\n\n正确做法是：多维度分析猜测范围（历史缺陷、开发习惯、业务复杂度、技术难点），有重点地猜测高风险区域，不是漫无目的地随机猜测。"
          },
          {
        ...
        items: [
          {
            ...
            answer: "AI 生成的断言可能看起来合理但实际错误。\n\n正确做法是：AI 生成初稿 → 人工复核业务语义 → 补充边界场景 → 运行验证。尤其是金额、状态、权限等关键字段，必须由人工确认。",
          },
          {
        ...
        items: [
          {
            ...
            answer: "AI 生成的用例是草稿，必须经过业务过滤。\n\n正确做法是：AI 生成候选用例 → 人工筛选业务相关场景 → 补充业务规则验证 → 形成最终用例集。尤其是业务特有的规则和边界，AI 可能无法理解。",
          },
          {
        ...
        items: [
          {
            ...
            answer: "AI 生成的文档可能有事实错误、表述不清或格式问题。\n\n正确做法是：AI 生成初稿 → 人工审核业务术语和关键结论 → 调整结构和表述 → 发布。尤其是业务相关的描述，必须由人工确认。",
          },
          {
