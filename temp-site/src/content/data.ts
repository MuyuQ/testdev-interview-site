      ...
      // ========== 模块 5：面试问答 ==========
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "重试和幂等是互补关系。重试是调用方的容错策略：当请求失败（超时、网络错误）时自动重试，提高请求成功率。幂等是被调用方的安全保障：保证重复请求不会产生副作用。两者配合才能实现可靠的分布式调用：调用方放心重试，被调用方安全处理重复请求。\n\n面试时可以举例：支付网关会重试发送回调，我们服务端做幂等处理，这样即使网络不好导致多次回调，也不会重复扣款。",
          },
        ],
      },
    ],
        ...
        items: [
          {
            ...
            answer: "两者是不同维度的概念。回归测试关注「变更影响」，目的是验证原有功能没有被新改动破坏；冒烟测试关注「基本可运行性」，目的是快速判断系统是否达到可测试状态。冒烟测试通常作为回归测试的前置关卡：先跑冒烟验证基础功能，冒烟通过再跑深度回归。冒烟测试用例较少（10-20 条），回归测试用例可能成百上千。冒烟测试更像是一种「快速检查」，回归测试是「深度验证」。\n\n面试时可以说：冒烟测试回答「系统能不能继续测」，回归测试回答「变更有没有影响老功能」。",
          },
          {
        ...
        items: [
          {
            ...
            answer: "分组不随机会导致选择性偏差，结果不可信。比如让活跃用户看新版本，新版本数据自然更好，但这不是方案更好，而是用户群体不同。\n\n正确做法：使用用户 ID 哈希后取模分组，分组算法不可人为干预；分组后验证各组用户特征分布是否一致（年龄、性别、活跃度分布相似）；如果分布不一致，使用分层抽样保证各组特征分布一致。\n\n面试时要强调：随机分组是 A/B 测试可信的前提，必须严格保证。",
          },
          {
            question: "误区二：样本量太小，结果不可信",
            answer: "样本量太小会导致统计显著性不足，结果可能只是随机波动。比如只测试 100 个用户就下结论，误差极大。\n\n正确做法：实验前计算最小样本量（根据期望提升效果、显著性水平、统计功效计算）；实验运行到达到最小样本量后才能分析结果；如果流量小要延长实验时间，不能提前结束。样本量估算示例：期望点击率从 5% 提升到 5.5%，显著性水平 0.05，统计功效 80%，每组需要约 15000 样本。面试时要说明：小样本的 A/B 测试结果不可信，必须保证样本量足够。",
          },
          {
            question: "误区三：过早结束实验或反复查看结果",
            answer: "过早结束实验（看到显著差异就停止）会导致假阳性；反复查看结果（每天看是否显著）会导致 p 值失效。这两种情况都会让结果不可信。\n\n正确做法：实验前确定样本量和运行时长，严格执行不提前结束；使用顺序检验（Sequential Testing）方法，允许在实验过程中查看结果但控制假阳性率；只有在达到预设条件后才能下结论。面试时要说明：A/B 测试要有严格的实验设计，不能边跑边看、随意决策。",
          },
          {
            question: "误区四：忽略辛普森悖论",
            answer: "辛普森悖论是指分组数据呈现的趋势与合并数据相反。比如各细分群体中 B 方案都更好，但合并后 A 方案更好，这是因为两组用户群体结构不同。\n\n正确做法：分析结果时要分层分析，不能只看整体数据；如果发现分层结果与整体结果不一致，要深入分析用户群体结构差异；报告结果时要同时呈现整体数据和分层细分数据。面试时要说明：辛普森悖论是 A/B 测试分析的常见陷阱，必须通过分层分析来避免错误结论。",
          },
          {
            question: "误区五：多个实验同时进行互相干扰",
            answer: "多个实验同时进行可能互相干扰，导致无法判断指标变化由哪个实验引起。比如同时测试新首页和新算法，用户行为变化无法归因。\n\n正确做法：设计分层实验框架，不同层实验正交独立；同一层内实验串行执行，避免互相干扰；记录用户参与的实验列表，分析时考虑实验间的交互效应。面试时要说明：大型互联网公司都有分层实验平台，支持多团队并行实验，这是 A/B 测试工程化的核心能力。",
          },
        ],
      },
      // ========== 模块 5：面试问答 ==========
