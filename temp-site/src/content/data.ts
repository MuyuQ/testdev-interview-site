  ...
  },
  {
    ...
    sections: [
      // ========== 模块 1：基础入门 ==========
      {
        ...
        kind: "paragraph",
        content: [
          { type: "text", content: "幂等（Idempotency）是分布式系统设计中的重要概念，尤其在支付、订单、消息队列等高风险业务场景中必不可少。\n\n它的核心含义是：同一个请求重复执行多次，系统状态和返回结果保持一致或可控。换句话说，幂等保证重复操作不会产生额外的副作用。" },
          { type: "text", content: "\n\n" },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            question: "判断流程",
            answer: "首先问自己这个资源初始化成本高吗？高成本（数据库连接、浏览器启动）考虑大作用域。然后问不同用例需要独立状态吗？需要独立则用 function 或 class。\n\n最后问资源需要清理吗？需要清理则用 yield 确保清理逻辑执行。",
          },
          {
            question: "选择表格",
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: "text", content: "重试对系统的影响是双面的。正向看，重试提高了请求成功率，减少了因瞬时故障导致的业务失败。负向看，重试增加了后端压力，特别是故障恢复期可能形成重试风暴。\n\n生产环境建议：限制单机并发重试数、配合熔断器快速失败、重要接口降级到异步队列。" },
          { type: "text", content: "" },
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: "text", content: "结构化日志是现代日志体系的标配。传统文本日志便于人眼阅读，但难以被程序解析和分析。结构化日志（通常为 JSON 格式）让每条日志都成为可检索、可聚合的数据对象。\n\n实践建议：使用统一的日志库输出 JSON 格式；定义标准字段集和命名规范；扩展字段使用嵌套结构避免扁平化污染；接入日志平台后可实现秒级检索、统计聚合、异常检测。" },
          { type: "text", content: "" },
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "我的方法是「日志 - 指标 - 链路」三步走：先看日志，找到报错信息和堆栈；再看指标，确认是否有性能异常（CPU、内存、响应时间）；\n\n最后看链路，追踪跨服务调用找到根因。三者结合能快速定位大部分问题。",
          },
        ],
      },
    ],
