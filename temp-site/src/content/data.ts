        ...
        items: [
          {
            ...
            answer: "两者的目的不同。A/B 测试是产品实验方法，目的是对比方案效果、验证产品假设、用数据驱动决策，关注「哪个方案更好」。灰度发布是技术发布策略，目的是降低发布风险、逐步放开新版本范围、发现问题快速回滚，关注「发布是否稳定」。\n\n实际工作中两者可以配合：先灰度发布到小范围流量观察稳定性，同时在灰度流量内做 A/B 测试验证效果，两者结合实现「稳定发布+效果验证」。",
          },
          {
          ...
          },
          {
            ...
            answer: "A/B 测试只测试单一变量的两个版本（如按钮颜色的红 vs 蓝），多变量测试同时测试多个变量的组合效果（如按钮颜色+按钮位置+按钮文案的所有组合）。A/B 测试简单直接、样本量要求低、易于分析，适合验证单一改动。多变量测试能发现变量间的交互效应，但需要更大的样本量（组合数多）、分析更复杂。\n\n实际工作中，A/B 测试更常用，多变量测试适用于精细化优化的成熟产品。",
          },
        ],
      },
      {
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            question: "为什么错",
            answer: "UI 自动化失败原因复杂：元素变化、网络问题、时序问题、环境问题。没有足够的信息，定位问题效率极低。\n\n尤其在 CI 环境中，无法重现问题，导致用例被标记为不稳定用例而忽略。",
          },
          {
            question: "正确做法",
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            question: "如何选择",
            answer: "数据驱动适合：输入输出组合多、逻辑相对固定、需要快速覆盖大量数据场景。关键字驱动适合：测试步骤需要灵活编排、非技术人员参与用例编写、构建测试平台。\n\n实际项目中，两者可以结合使用：关键字驱动处理流程编排，数据驱动处理参数组合。",
          },
        ],
      },
      {
