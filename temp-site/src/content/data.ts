    ...
    sections: [
      {
        ...
        content: [
          ...
          { type: "text", content: "\n\n" },
          { type: "text", content: "Stub（桩）是最简单的测试替身，它只做一件事：在被调用时返回预设的固定值。Stub 不关心被调用了多少次、以什么参数调用，它只是被动地提供数据。\\n\\n比如一个返回用户信息的 Stub，无论传入什么用户 ID，都返回一个固定的用户对象。Stub 的特点是「只提供数据，不验证行为」。" },
          { type: "text", content: "\n\n" },
          { type: "text", content: "Mock（模拟对象）则更进一步，它不仅能返回预设值，还能记录调用行为。Mock 可以验证方法是否被调用、调用次数、调用参数等交互信息。\\n\\n比如验证「支付服务的 pay 方法是否被调用了一次，且传入的金额参数是否正确」。Mock 的特点是「验证行为交互」。" },
          { type: "text", content: "\n\n" },
    ...
    sections: [
      ...
      // ========== 模块 3：实操案例 ==========
      {
        ...
        content: [
          ...
          { type: "text", content: "\n\n" },
          { type: "text", content: "面试表达要点：强调数据驱动让测试逻辑与数据解耦，新增测试用例只需添加数据行，无需修改测试代码。\n\n同时数据文件可独立评审、版本管理，测试代码更简洁。" },
        ],
      },
      {
    ...
    sections: [
      // ========== 模块 1：基础入门 ==========
      {
        ...
        content: [
          ...
          { type: "text", content: "\n\n" },
          { type: "text", content: "性能测试的核心指标有五：响应时间（用户体验）、吞吐量（系统处理能力）、并发数（并发处理能力）、错误率（稳定性）、资源利用率（瓶颈定位）。这些指标要综合分析，不能只看单一指标。\\n\\n比如响应时间达标但错误率高，说明系统不稳定；吞吐量高但资源利用率超限，说明系统接近瓶颈。" },
          { type: "text", content: "\n\n" },
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "语句覆盖率衡量有多少代码行被执行，分支覆盖率衡量有多少条件分支被执行（如 if/else 的两个分支）。语句覆盖率容易达到但覆盖浅，分支覆盖率更难但覆盖深。\\n\\n比如 if-else 语句，语句覆盖率只要执行其中一个分支就达标，分支覆盖率需要执行两个分支才达标。",
          },
          {
        ...
        items: [
          {
            ...
            answer: "正确做法：覆盖率只是参考指标，更重要的是测试质量和关键路径覆盖。代码覆盖率高不代表测试质量好，可能只是执行了代码路径但没做有效断言。要配合「有效断言」「深度测试」「关键路径覆盖」才有意义。\\n\\n举例说明高覆盖率但低质量的场景（如测试只执行代码不做断言）。",
          },
          {
    ...
    sections: [
      {
        ...
        title: "基础入门",
        kind: "paragraph",
        content: text("等价类划分是黑盒测试中最基础、最实用的测试设计方法，核心思想是将输入域划分成若干个等价类，每个等价类中的所有值对于测试目的来说是等效的。测试时只需从每个等价类中选取一个代表值进行测试，假设该等价类中的其他值会得到相同的测试结果。等价类分为两类：有效等价类是指符合需求和规格说明的合法输入集合，无效等价类是指不符合需求和规格说明的非法输入集合。\\n\\n例如用户名输入框限制为 6-20 位字母数字，有效等价类是 6-20 位字母数字组合，无效等价类包括长度小于 6、长度大于 20、含特殊字符、空值等多种情况。等价类划分的核心价值在于大幅减少测试用例数量，避免冗余测试，同时保证测试覆盖率。"),
      },
      {
        id: "why-important",
        title: "为什么重要",
      ...
      },
      {
        ...
        title: "面试问答",
        kind: "qa-list",
        items: [{question: "什么是等价类划分？为什么能减少测试用例数量？", answer: "等价类划分是将输入域分成若干等价类，每类选代表值测试。减少用例的原因是：假设等价类内所有值测试结果相同，无需穷举测试每个值。\\n\\n例如输入范围 1-100，传统方法需测 100 次，等价类划分只需测「有效类」「无效类」两个代表值。"}, {question: "有效等价类和无效等价类怎么划分？", answer: "有效等价类：收集符合规则的合法输入，范围限制归一类，枚举值各一类。无效等价类：每种非法情况独立一类，包括范围外、格式错、类型错、空值等。关键原则：有效等价类可组合测试，无效等价类必须单独测试。"}, {question: "等价类划分设计测试用例的步骤是什么？", answer: "第一步：分析输入条件，从需求中提取所有输入约束。\n\n第二步：划分等价类，每个条件分成有效和无效等价类。\n\n第三步：选代表值，每个等价类选典型值。\n\n第四步：组合测试用例，有效等价类组合成有效用例，无效等价类单独设计无效用例。"}, {question: "等价类划分和边界值分析怎么配合使用？", answer: "配合原则：等价类划定范围，边界值深入边界。先做等价类划分确定测试范围（每个等价类至少测一个），再做边界值分析在边界附近深入测试（边界-1、边界、边界+1）。\\n\\n例如范围 1-100：等价类确定测「有效」「无效」两类，边界值补充测 0、1、100、101 四个关键点。"}, {question: "举一个等价类划分发现线上问题的案例？", answer: "案例：优惠券系统限制「每人限领 3 张」。等价类划分为：有效类（已领 0、1、2 张）、无效类（已领 3 张以上）。测试发现：用户已领 3 张后仍可领取，原因是开发用「小于」而非「小于等于」判断。这个缺陷通过等价类划分的边界点测试发现。"}],
      },
    ],
  },
  {
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "我常用的探索方法有六种：角色探索（扮演不同用户角色体验系统，比如买家下单、商家发货、客服处理投诉）、场景探索（模拟真实用户场景，比如促销活动期间抢购、多人拼单、跨店铺购物）、特性探索（深入探索某个特性，比如优惠券的各种使用限制、叠加规则）、数据探索（各种数据输入组合，比如不同商品类型、价格范围、库存状态）、边界探索（边界值和临界条件，比如库存刚好为0、支付金额刚好超限）、破坏性探索（故意破坏流程，比如支付中途取消、网络中断重连、重复点击提交）。\\n\\n举例：探索订单模块时，我用角色探索从买家角度体验完整购物流程，发现了搜索结果排序不一致的问题；用破坏性探索测试支付中断，发现了订单状态卡在待支付的问题。",
          },
        ],
      },
    ],
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: "text", content: "\n\n" },
          { type: "text", content: "问题分析：如果所有实验都简单按用户 ID 取模分组，用户 ID % 100 < 50 可能同时被分配到多个实验的实验组，导致无法区分指标变化是由哪个实验引起的。\\n\\n比如用户同时看到「新首页+新搜索算法+新支付流程」，如果这个用户的转化率提升了，无法判断是哪个改动带来的。" },
          { type: "text", content: "\n\n" },
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: "text", content: "**Fixture 准备**：执行测试前，conftest.py 中的 Fixture 按作用域和依赖关系初始化。\\n\\n比如 db fixture 准备数据库连接，api_client fixture 准备请求客户端。\n\n" },
      ...
      },
      {
        ...
        content: [
          ...
          { type: "text", content: "第三步：设置超时时间，避免无限等待。\\n\\n比如最多等待 60 秒，超过则判定失败。\n\n" },
      ...
      },
      {
        ...
        content: [
          ...
          { type: "text", content: "第一步：识别需要 Mock 的依赖。\\n\\n比如支付模块调用的第三方支付网关、短信服务、邮件通知。\n\n" },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          {
            question: "错误表现",
            answer: "每个命令单独执行，不知道如何把多个命令组合起来。\\n\\n比如查看进程时，不知道怎么过滤关键字。",
          },
          {
            question: "为什么错",
        ...
        items: [
          {
            ...
            answer: "按影响范围和严重程度分级：P0 级是核心报表数据错误、数据丢失，需要立即响应；P1 级是关键指标偏差超阈值、任务链路中断，需要当天处理；P2 级是非核心数据异常、数据延迟，可排期处理。\n\n同时建立数据质量评分机制，定期复盘改进。",
          },
          {
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "【优先级模型】按影响面和严重程度两个维度划分：P0 阻塞级（核心功能不可用、影响用户占比 >10%）立即修复；P1 严重级（核心功能受限、影响用户占比 5%-10%）当前版本修复；P2 一般级（体验问题、影响用户占比 1%-5%）排期修复；P3 轻微级（边缘场景、影响用户占比 <1%）视情况修复。【决策依据】用户数据是核心依据：用户占比越高的环境问题优先级越高；功能重要性是关键因素：支付相关 > 核心业务 > 辅助功能；问题严重程度是底线：功能阻塞 > 交互受阻 > 视觉问题。【成本考量】修复成本与收益评估：一个影响 5% 用户的阻塞问题可能比影响 50% 用户的视觉问题更紧急；\n\n同时评估是否可以通过引导用户升级浏览器或系统来解决，降低修复成本。",
          },
          {
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: "text", content: "1. 可重试判断器：接收异常对象，根据异常类型和状态码返回是否可重试。\\n\\n例如：连接超时返回 true，认证失败返回 false。" },
        ...
        items: [
          ...
          },
          {
            ...
            answer: [
              ...
              { type: "text", content: "\n" },
              { type: "text", content: "回答思路：「我们建立了反馈机制。每次 Review 结束，开发者可以标记 AI 的判断是否有效。我们定期分析误报原因，调整规则和 Prompt。\\n\\n比如发现 AI 经常误报某个工具类的使用方式，我们就加一条规则排除这种情况。持续优化三个月后，准确率从 50% 提升到了 70%。」" },
              { type: "text", content: "\n\n" },
