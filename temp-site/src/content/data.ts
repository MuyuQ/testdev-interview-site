      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer: "遇到过订单并发状态冲突的问题。用户在手机端取消订单，同时商家在后台发货，最终订单状态变成已发货但用户已经取消。\n\n原因是状态更新没有前置校验，后执行的操作覆盖了先执行的操作。\n\n通过状态转换测试发现了这个问题：用JMeter模拟取消和发货同时请求，发现状态不一致。解决方案是引入乐观锁，发货时检查当前状态必须是已支付才能发货，否则拒绝。同时增加了分布式锁，确保同一订单同一时间只有一个操作能执行。\n\n这个案例让我深刻理解了状态转换测试要覆盖并发场景的重要性。",
          },
          {
