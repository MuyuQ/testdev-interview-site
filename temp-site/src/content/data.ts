  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        title: "面试问答",
        kind: "qa-list",
        items: [{question: "什么是边界值分析？为什么边界附近容易出问题？", answer: "边界值分析是一种黑盒测试设计方法，核心思想是「缺陷多发生在边界附近」。边界附近容易出问题的原因包括：比较运算符错误、逻辑判断遗漏、隐含边界被忽略。"}, {question: "边界值分析的测试点怎么设计？", answer: "遵循「三值原则」：测试「边界值、边界-1、边界+1」三个点。\n\n步骤：识别边界、设计三值测试点、明确预期结果。"}, {question: "边界值分析和等价类划分怎么配合使用？", answer: "等价类划分是「宏观划分」，边界值分析是「微观深入」。先用等价类划分确定测试范围，再用边界值分析补充边界测试点。"}, {question: "举一个边界值导致线上问题的例子？", answer: "典型案例：支付金额边界问题。促销活动「满 100 减 10」，用户输入 99.99 元时优惠没生效，原因是开发判断条件写成「金额 >= 100」只考虑整数。"}, {question: "怎么识别隐含边界？", answer: "识别方法：查数据库设计、查 API 文档、查系统配置、查历史缺陷、跨系统边界分析。"}],
      },
    ],
  },
  {
    ...
    sections: [
      ...
      // ========== 模块 3：实操案例 ==========
      {
        ...
        content: [
          ...
          { type: "text", content: "\n\n" },
          { type: "text", content: "结果分析：运行 7 天后，A 组点击率 5.02%，B 组点击率 5.48%，p 值 0.03 小于 0.05，差异具有统计显著性。\n\n结论：红色按钮点击率显著高于蓝色按钮，建议全量切换为红色按钮。" },
        ],
      },
      {
        id: "case-layers",
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            question: "消息重复消费怎么测？",
            answer: "模拟重复投递场景：消费者重复收到同一消息、网络抖动导致 ACK 丢失、消费者重启后重新消费。验证点：消费逻辑是否幂等、是否会重复写入数据、业务状态是否一致。\n\n测试方法：手动发送重复消息、延迟 ACK、模拟网络问题。",
          },
          {
    ...
    sections: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: "text", content: "可配置的重试策略让系统更灵活。核心配置项：最大重试次数、退避策略类型（固定/线性/指数）、初始间隔、最大间隔、可重试异常列表、总体超时时间。\n\n配置方式：代码默认值 + 配置文件覆盖 + 动态调整（结合配置中心）。不同接口可有不同策略，例如支付接口重试次数少但间隔短，数据同步接口重试次数多但间隔长。" },
          { type: "text", content: "" },
